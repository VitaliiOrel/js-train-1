...
function isEvenNumber(num) {
  if (num % 2 !== 0) {return false} // Перевірити, чи число num є парним
  return true // Повернути результат перевірки (true або false) як результат функції
}
...
function calculateSum(n) {
  ...
  for (i = 1; i <= n; i++) { // Запустити цикл для змінної i від 1 до n
...
function isPrimeNumber(num) {
  ...
  for (i = 2; i < num; i++) {// Запускаємо цикл для змінної i від 2 до num-1
...
function isPositiveNumber(num) {
  if (num > 0) {return true}// Перевіряємо, чи число num більше 0
...
// Задача 6: Напишіть функцію checkDataType, яка приймає параметр value і перевіряє 
//його тип даних. Функція повинна використовувати оператор typeof для визначення типу значення.
// Виведіть в консоль повідомлення з результатом перевірки типу, 
//наприклад, Значення "hello" має тип "string"

function checkDataType(value) {
  let typeValue = typeof(value)// Отримуємо тип значення value за допомогою оператора typeof
  console.log(`The value ${value} is of type "${typeValue}"`)// Виводимо у консоль повідомлення, яке містить значення value та його тип
  return typeValue// Повертаємо тип значення value
}
...
// Задача 7: Напишіть функцію findMax, яка приймає два числа num1 і num2 і повертає 
//більше з них. Використовуйте умовний оператор if та оператор порівняння для визначення більшого числа.
...
// Задача 8: Напишіть функцію getUserStatus, яка приймає вік користувача age і повертає 
//рядок, що вказує на його статус.
// Якщо вік менше 18 років, функція повертає "Неповнолітній". Якщо вік від 18 до 65 років, 
//функція повертає "Дорослий". Якщо вік більше або дорівнює 65 рокам, функція повертає "Пенсіонер".
// Використовуйте умовний оператор if, else if та оператори порівняння для визначення 
//статусу користувача.

function getUserStatus(age) {
  if (age < 18) {return "Неповнолітній"}// Перевіряємо вік за допомогою умовних операторів
  else if (age >= 18 && age < 65) {return "Дорослий"}// Якщо age менше 18, то користувач є "Неповнолітнім"
  else if (age >= 65) {return "Пенсіонер"}// Якщо age більше або дорівнює 18 і менше 65, то користувач є "Дорослим"
...
function getDayOfWeek(dayNumber) {
  let dayTitle// Використовуємо оператор switch для знаходження назви дня тижня залежно від dayNumber
  switch (dayNumber) {// В кожному випадку (case) перевіряємо значення dayNumber і присвоюємо змінній day відповідну назву дня тижня
  case 1: dayTitle = "Понеділок" 
  break// Якщо dayNumber = 1, то day = "Понеділок"
  case 2: dayTitle = "Вівторок" 
  break// Якщо dayNumber = 2, то day = "Вівторок"
  case 3: dayTitle = "Середа" 
  break// Якщо dayNumber = 3, то day = "Середа"
  case 4: dayTitle = "Четвер" 
  break// Якщо dayNumber = 4, то day = "Четвер"
  case 5: dayTitle = "П'ятниця" 
  break// Якщо dayNumber = 5, то day = "П'ятниця"
  case 6: dayTitle = "Субота" 
  break// Якщо dayNumber = 6, то day = "Субота"
  case 7: dayTitle = "Неділя" 
  break// Якщо dayNumber = 7, то day = "Неділя"
  default: dayTitle = "Невірний номер дня"// Якщо значення dayNumber не співпадає з жодним з випадків, то day = "Невірний номер дня"
  }
  return dayTitle// Повертаємо назву дня тижня, яку зберегли у змінній day
}
...
// Задача 10: Напишіть функцію getVariableType, яка приймає змінну variable і повертає 
//рядок, що відповідає типу цієї змінної. Використовуйте тернарний оператор для визначення 
//типу змінної.

function getVariableType(variable) {
  let message
  let messageOptions = {number: 'Це число', string: 'Це рядок', boolean: 'Це булеве значення',}
  let typeOfVariable = typeof(variable)
  typeOfVariable == 'number' || typeOfVariable == 'string' || typeOfVariable == 'boolean'
  ? message = messageOptions[typeOfVariable]
...
function factorial(n) {
  if (n === 0 || n === 1) 
    ...
    if (n > 1) {n *= factorial(n-1)} // Якщо n більше за 1, ми множимо n на результат виклику тієї самої функції з n-1.
    return n 
}
...
// Задача 12: створити функцію `makeAdder`, яка приймає число `x` і 
//повертає нову функцію, що приймає число `y` і повертає суму `x` та `y`.
function makeAdder(x) {
  return y => x + y// Вертаємо нову функцію, яка приймає `y`
...
// Задача 13: Задача: створити стрілкову функцію, яка приймає два числа і 
//повертає їх добуток.
...
function divide(x) {
  return y => x / y// Повертаємо нову функцію, яка приймає `y`
...
// Задача 15: Створити мемоізовану версію функції, яка обчислює площу квадрата на 
//основі довжини його сторони.

function memoizedArea() {
  const cache = {}
  return function (side) {
    if (typeof(side) !== 'number' || side <= 0) {return 'Error'}
    if (cache[side] !== undefined) {
      console.log(`Fetching from cache for ${side}`)
      return cache[side]
    } else {
      const result = side * side
      cache[side] = result
      console.log(`Calculating for ${side}`)
      return result
    }
  }
}
...
function compose(func1, func2) {
  return n => {
   return func2(func1(n))// Повертаємо нову функцію, яка приймає аргумент n
  }// Повертаємо func2, в яку передаємо func1 з аргументом n
}

const createCompose = compose(increment, double)
// Створюємо змінну createCompose якій присвоємо результат виконання функції compose 
//з аргументами increment та double
